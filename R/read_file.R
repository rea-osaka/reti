...
#'   function take to finish work.
#' @return A tibble data.
#' @examples
#' \dontrun{
#' osaka <- reti_readBase("csvdata/27_osaka_Prefecture_20132_20181.csv")
#' }
#' @export
reti_readBase <- function(path, timecount = TRUE)
{
    # 時間計測用処理
    t_start <-proc.time()
    ...
    df <- dplyr::mutate(df,
                        ...
                        t_year = lubridate::year(t_date),
                        t_qtr  = lubridate::quarter(t_date)
                        )
